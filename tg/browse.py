...
def browse_books_navigator(_: Client, clb: CallbackQuery):
    ...
    if next_prev_buttons:
        buttons.append(
            next_prev_buttons if Language.from_code(clb.from_user.language_code).rtl else next_prev_buttons[::-1]
        )
    ...
    clb.edit_message_text(
        text=gs(user_id=clb.from_user.id, string=s.X_TO_Y_OF_TOTAL,
